...
interface SentimentResponse {
  ...
  agentBehaviorScore?: number | null
...
export function InteractionDrawer({
  ...
  const [agentBehaviorScore, setAgentBehaviorScore] = useState<number | null>(
    null,
  )
  ...
  useEffect(() => {
    ...
    const interactionId = interaction.call_id as string
    ...
    ;(async () => {
      try {
        ...
        if (typeof data.agentBehaviorScore === "number") {
          setAgentBehaviorScore(data.agentBehaviorScore)
        }

        if (Array.isArray(data.customerSatisfaction)) {
          setTelephoneSkills(
            data.customerSatisfaction.map((m) => ({
              label: m.label,
  ...
  const handleCheckChange = (id: string, checked: boolean) => {
    setCriteria((prev) =>
      prev.map((criterion) =>
        criterion.id === id ? { ...criterion, checked } : criterion,
      ),
    )
  }
  ...
  const handleNotesChange = (id: string, notes: string) => {
    setCriteria((prev) =>
      prev.map((criterion) =>
        criterion.id === id ? { ...criterion, notes } : criterion,
      ),
    )
  }
            ...
            <div className="border-b border-border bg-gradient-to-br from-muted/50 via-muted/30 to-background px-6 py-4 shrink-0">
              <div className="rounded-lg bg-muted/1 p-3">
                <p className="text-sm text-muted-foreground leading-relaxed">
                  <strong>Audio Player:</strong> Play the call recording at any
                  point in time by clicking over the audio track.
                </p>
              </div>
                ...
                {activeTab === "scores" && (
                  <div className="space-y-8">
                    ...
                    <div>
                      ...
                      ) : telephoneSkills.length === 0 ? (
                        <div className="text-sm text-muted-foreground">
                          No scoring data available for this call.
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {telephoneSkills.map((item, index) => (
                            <div
                              key={index}
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="rounded-lg bg-muted/50 p-8">
                      {sentimentLoading ? (
                        ...
                        <div className="text-sm text-red-400">
                          Could not load sentiment: {sentimentError}
                        </div>
                      ) : (
                        <>
                    ...
                    {!sentimentLoading && !sentimentError && (
                      <div className="space-y-4 text-base">
                        <p className="leading-relaxed text-muted-foreground">
                          The conversation shows a {effectiveSentiment} sentiment
                          with a confidence score of {effectiveSentimentScore}%.
                        </p>
                      </div>
                    )}
                  </div>
